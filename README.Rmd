...
**orcidtr** provides a modern, CRAN-compliant R interface to the [ORCiD public API](https://info.orcid.org/documentation/api-tutorials/). It replaces the discontinued `rorcid` package with a streamlined implementation that:
...
[ORCiD](https://orcid.org) (Open Researcher and Contributor ID) is a persistent digital identifier that distinguishes individual researchers and supports automated linkages between researchers and their professional activities. An ORCiD record can contain employment history, education, publications, funding, peer review activities, and more.
...
**Important:** The ORCID public API allows unauthenticated read access to all public data. Authentication is entirely optional and only needed for:
...
1. Register for ORCID API credentials at https://orcid.org/developer-tools
...
**Note:** No function reads `ORCID_TOKEN` from the environment. If you keep a token in `.Renviron`, retrieve it with `Sys.getenv("ORCID_TOKEN")` and pass it through. For the public API this is optional, and most users can skip this section entirely.
...
- Add delays for large batch operations: `Sys.sleep(0.1)` between requests
...
- Parts of this package were written with the assistance of Claude Code (Anthropic); all code has been reviewed and tested by the author
