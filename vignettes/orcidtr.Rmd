...
# Display summary
n_articles <- sum(works$type == "journal-article", na.rm = TRUE)
cat(sprintf("Total works: %d\n", nrow(works)))
cat(sprintf("Journal articles: %d\n", n_articles))

# View recent publications
recent_works <- works[order(-publication_date)][
  1:5, .(title, type, publication_date, doi)
]
print(recent_works)
...
Every function that accepts a token takes it as an argument. No function reads one from the environment, so a token you keep in `.Renviron` has to be retrieved and passed through yourself.

```{r auth-usage}
# Pass the token explicitly
works <- orcid_works("0000-0003-4757-117X", token = "your-token-here")

# Or read it from your own environment variable
works <- orcid_works("0000-0003-4757-117X", token = Sys.getenv("ORCID_TOKEN"))
```
